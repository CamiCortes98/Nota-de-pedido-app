...
class SumarizadorNotasPedido:
    def __init__(self, root):
        ...
        self.cal_fecha_inicio = DateEntry(self.root, width=12, background='darkblue', foreground='white')
        self.cal_fecha_inicio.pack()

        self.lbl_fecha_fin = tk.Label(self.root, text="Seleccione fecha de fin:")
        self.lbl_fecha_fin.pack()
        self.cal_fecha_fin = DateEntry(self.root, width=12, background='darkblue', foreground='white')
        ...
        self.entry_laboratorio = ttk.Combobox(self.root, textvariable=self.selected_laboratorio)
        self.entry_laboratorio['values'] = self.laboratorios
    ...
    def sumarizar(self):
        ...
        try:
            ...
            df_filtrado = self.df[
                (self.df["Fecha del pedido"].dt.date >= fecha_inicio) &
                (self.df["Fecha del pedido"].dt.date <= fecha_fin) &
                (self.df["Laboratorio"] == laboratorio)
            ]
            ...
            resumen_con_total = pd.concat([resumen.reset_index(), pd.DataFrame({"Codebar": ["TOTAL", ""], "Producto": [None, None], "Cantidad": [total_cantidades, None], "Imp. Total": [None, importe_total_total]}, index=[""] * 2)])
            
            # Eliminar el .0 del final en el Codebar
            resumen_con_total["Codebar"] = resumen_con_total["Codebar"].astype(str).str.rstrip('.0')
